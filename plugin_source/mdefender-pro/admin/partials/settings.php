...
if (empty($waf_dash_url) || strpos($waf_dash_url, 'onrender.com') !== false) {
    $waf_dash_url = 'http://localhost:5173';
}
...
?>
    ...
    <style>
        ...
        .waf-cloud-icon-box .dashicons {
            font-size: 22px;
            width: 22px;
            height: 22px;
        }
        .waf-cloud-capabilities {
            display: flex;
            gap: 8px;
            flex-wrap: wrap;
            margin-bottom: 18px;
        ...
        .waf-cap-pill .dashicons {
            font-size: 14px;
            width: 14px;
            height: 14px;
            color: #2563eb;
        }
        .waf-key-input-wrapper {
            position: relative;
            display: flex;
            align-items: center;
            margin-top: 6px;
        ...
        .waf-toggle-key-visibility:hover {
            color: #1e293b;
            background: #f1f5f9;
        }
        .waf-key-hints {
        ...
        .waf-btn-connect:hover {
            background: linear-gradient(135deg, #1d4ed8 0%, #1e40af 100%);
            box-shadow: 0 4px 14px rgba(37, 99, 235, 0.32);
            transform: translateY(-1px);
        }
        .waf-btn-connect:active {
        ...
        .waf-connect-badge.ok {
            background: #ecfdf5;
            color: #047857;
            border: 1px solid #a7f3d0;
        }
        .waf-connect-badge.bad {
            background: #fef2f2;
            color: #b91c1c;
            border: 1px solid #fecaca;
        }
        ...
        .waf-connect-badge.ok .waf-status-dot {
            background: #10b981;
            box-shadow: 0 0 0 3px rgba(16, 185, 129, 0.2);
            animation: wafPulse 2s infinite;
        }
        .waf-connect-badge.bad .waf-status-dot {
            background: #ef4444;
        }
        @keyframes wafPulse {
    ...
    <div class="waf-settings">
        ...
        <div id="waf-tab-general" class="waf-tab-panel">
            <div class="waf-settings-grid">
                ...
                <div class="waf-fw-card waf-span-2 waf-cloud-card">
                    <div class="waf-cloud-header">
                        <div class="waf-cloud-title-wrap">
                            <div class="waf-cloud-icon-box">
                                <span class="dashicons dashicons-shield"></span>
                            </div>
                            <div>
                                <h3 style="margin:0;font-size:16px;color:#0f172a;"><span class="dashicons dashicons-cloud" style="color:#2563eb;font-size:18px;"></span> MDefender-Pro Cloud Service</h3>
                                <p class="description" style="margin:3px 0 0;font-size:12.5px;">Machine Learning WAF &amp; Enterprise Cloud Malware Scanning Network.</p>
                            </div>
                        </div>
                        <div id="wafCloudBadge" class="waf-connect-badge bad">
                            <span class="waf-status-dot"></span>
                            <span class="waf-status-text">Not connected</span>
                        </div>
                    </div>

                    <div class="waf-cloud-capabilities">
                        <div class="waf-cap-pill"><span class="dashicons dashicons-superhero"></span> Cloud AST WAF Engine</div>
                        <div class="waf-cap-pill"><span class="dashicons dashicons-search"></span> 27k+ Malware Hashes</div>
                        <div class="waf-cap-pill"><span class="dashicons dashicons-randomize"></span> Realtime Threat Sync</div>
                    </div>

                    <form id="wafFwMlForm" class="waf-cloud-form">
                        <div class="waf-api-key-group" style="margin-bottom: 14px;">
                            <label for="wafFwMlUrl" style="font-weight:600;font-size:13px;color:#1e293b;">Cloud Server Endpoint / URL</label>
                            <div class="waf-key-input-wrapper">
                                <span class="dashicons dashicons-admin-site-alt3 waf-key-lead-icon"></span>
                                <input type="text" id="wafFwMlUrl" class="waf-key-input" placeholder="http://217.15.170.82" value="<?php echo esc_attr(get_option('waf_fw_ml_api_url', 'http://217.15.170.82')); ?>" autocomplete="off">
                            </div>
                            <span class="description" style="font-size:12px;color:#64748b;margin:4px 0 0;display:block;"><span class="dashicons dashicons-info" style="font-size:14px;vertical-align:middle;"></span> Your MDefender-Pro Cloud VPS IP or Domain (e.g. <code>http://217.15.170.82</code>).</span>
                        </div>

                        <div class="waf-api-key-group">
                            <label for="wafFwMlKey" style="font-weight:600;font-size:13px;color:#1e293b;">Website API Key</label>
                            <div class="waf-key-input-wrapper">
                                <span class="dashicons dashicons-admin-network waf-key-lead-icon"></span>
                                <input type="password" id="wafFwMlKey" class="waf-key-input" placeholder="Enter your MDefender-Pro Website API Key (AM-...)" autocomplete="off">
                                <button type="button" class="waf-toggle-key-visibility" id="wafToggleKeyBtn" title="Toggle visibility">
                                    <span class="dashicons dashicons-visibility"></span>
                                </button>
                            </div>
                            <div class="waf-key-hints">
                                <span class="description" style="font-size:12px;color:#64748b;margin:0;"><span class="dashicons dashicons-info" style="font-size:14px;vertical-align:middle;"></span> Issued per-website in your MDefender-Pro Dashboard.</span>
                                <span id="wafConnectedSiteMeta" style="display:none;font-size:12px;color:#059669;font-weight:600;"><span class="dashicons dashicons-yes-alt" style="font-size:14px;vertical-align:middle;"></span> Protected &amp; Synced with Cloud</span>
                            </div>
                        </div>

                        <div class="waf-btn-row" style="display:flex;gap:10px;margin-top:16px;align-items:center;">
                            <button type="submit" class="waf-btn-connect" id="wafSaveMlBtn">
                                <span class="dashicons dashicons-cloud-saved"></span>
                                <span id="wafConnectBtnText">Save &amp; Connect</span>
                            </button>
                            <button type="button" class="waf-btn-disconnect" id="wafDisconnectMl" style="display:none;">
                                <span class="dashicons dashicons-no-alt"></span> Disconnect
                            </button>
                        </div>
                        <div id="wafMlTestResult" class="waf-test-result" style="display:none;margin-top:14px;"></div>
                    </form>
                </div>
    ...
    function wafTestMLConnection() {
        ...
        jQuery.post(ajaxurl + '?action=waf_fw_save_settings&nonce=' + waf_fw_ajax.nonce, {
            ml_api_key: key,
            test_connection: 1
        }, function(r) {
            if (r.success) {
                var html = '';
                if (r.data && r.data.connected === true) {
                    ...
                    var msg = (r.data && r.data.connection_message) ? r.data.connection_message : 'Could not reach the cloud service.';
                    html = '<div style="background:var(--war-red-light);border:1px solid #fecaca;border-radius:6px;padding:12px;color:#b91c1c;"><strong>Connection Failed:</strong> ' + msg + '<br><small>Check that the API key belongs to this website in your MDefender-Pro dashboard.</small></div>';
                    wafCloudBadge('no');
                }
                jQuery('#wafMlTestResult').html(html).show();
                jQuery.get(ajaxurl, {action:'waf_fw_get_settings'}, function(r2) {
                    if (r2.success) updateStatus(r2.data);
                });
            } else {
                var err = (r.data && r.data.message) ? r.data.message : 'Unknown error';
                jQuery('#wafMlTestResult').html('<div style="background:var(--war-red-light);border:1px solid #fecaca;border-radius:6px;padding:12px;color:#b91c1c;">Save failed: ' + err + '</div>').show();
            }
        }).fail(function(xhr) {
            var msg = 'Network error. Check your server connection.';
            if (xhr && xhr.responseJSON && xhr.responseJSON.data && xhr.responseJSON.data.message) {
                msg = xhr.responseJSON.data.message;
            }
            jQuery('#wafMlTestResult').html('<div style="background:var(--war-red-light);border:1px solid #fecaca;border-radius:6px;padding:12px;color:#b91c1c;">' + msg + '</div>').show();
        }).always(function() {
    ...
    function wafCloudBadge(connected) {
        ...
        var $btnText = jQuery('#wafConnectBtnText');
        var $meta = jQuery('#wafConnectedSiteMeta');
        if (connected === 'yes') {
            $b.attr('class', 'waf-connect-badge ok').html('<span class="waf-status-dot"></span><span class="waf-status-text">Connected to Cloud</span>');
            jQuery('#wafDisconnectMl').show();
            $btnText.text('Save & Reconnect');
            $meta.show();
        } else {
            $b.attr('class', 'waf-connect-badge bad').html('<span class="waf-status-dot"></span><span class="waf-status-text">Not connected</span>');
            jQuery('#wafDisconnectMl').hide();
            $btnText.text('Save & Connect');
            $meta.hide();
        }
    }
    ...
    jQuery(document).ready(function($) {
        ...
        $('#wafToggleKeyBtn').on('click', function() {
            var input = $('#wafFwMlKey');
        ...
        $('#wafDisconnectMl').on('click', function() {
            ...
            $.post(ajaxurl + '?action=waf_fw_save_settings&nonce=' + waf_fw_ajax.nonce, {
                ml_api_key: ''
            }, function(r) {
                $('#wafFwMlKey').val('');
                wafCloudBadge('no');
        ...
        $('#wafFwMlForm').on('submit', function(e) {
            ...
            var url = $('#wafFwMlUrl').val().trim() || 'http://217.15.170.82';
            if (!key) {
            ...
            btn.prop('disabled', true).html('<span class="dashicons dashicons-update" style="animation:wafSpin 1s infinite linear;display:inline-block;vertical-align:middle;"></span> Connecting...');
            ...
            jQuery.post(ajaxurl + '?action=waf_fw_save_settings&nonce=' + waf_fw_ajax.nonce, { 
              ...
              if (r.success) {
                  ...
                  if (r.data && r.data.connected === true) {
                      ...
                      var msg = (r.data && r.data.connection_message) ? r.data.connection_message : 'Could not reach cloud service.';
                      html = '<div style="background:var(--war-red-light);border:1px solid #fecaca;border-radius:6px;padding:12px;color:#b91c1c;"><strong>Connection Failed:</strong> ' + msg + '<br><small>Check that the API key is active in your MDefender-Pro dashboard.</small></div>';
                      wafCloudBadge('no');
                  }
                  jQuery('#wafMlTestResult').html(html).show();
                  jQuery.get(ajaxurl, {action:'waf_fw_get_settings'}, function(r2) {
                      if (r2.success) {
                          updateStatus(r2.data);
                          if (r2.data.ml_api_url) $('#wafFwMlUrl').val(r2.data.ml_api_url);
                      }
                  });
              } else {
                  var err = (r.data && r.data.message) ? r.data.message : 'Save failed';
                  jQuery('#wafMlTestResult').html('<div style="background:var(--war-red-light);border:1px solid #fecaca;border-radius:6px;padding:12px;color:#b91c1c;">' + err + '</div>').show();
              }
            }).fail(function(xhr) {
                var msg = 'Network error. Check your server connection.';
                if (xhr && xhr.responseJSON && xhr.responseJSON.data && xhr.responseJSON.data.message) {
                ...
                $('#wafMlTestResult').html('<div style="background:var(--war-red-light);border:1px solid #fecaca;border-radius:6px;padding:12px;color:#b91c1c;">' + msg + '</div>').show();
            }).always(function() {
                var isConn = $('#wafDisconnectMl').is(':visible');
                btn.prop('disabled', false).html('<span class="dashicons dashicons-cloud-saved"></span> <span id="wafConnectBtnText">' + (isConn ? 'Save &amp; Reconnect' : 'Save &amp; Connect') + '</span>');
            });
        });
        ...
        $('[id$=Form]').not('#wafFwMlForm, #wafFwScheduleForm').on('submit', function(e) {
            e.preventDefault();
            var form = $(this);

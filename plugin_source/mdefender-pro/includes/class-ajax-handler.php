...
class WAF_FW_Ajax_Handler {
    public function __construct() {
        ...
        add_action('wp_ajax_waf_fw_diff_core_file', [$this, 'diff_core_file']);
        add_action('wp_ajax_waf_fw_bulk_restore_core_files', [$this, 'bulk_restore_core_files']);
        add_action('wp_ajax_waf_fw_bulk_clean_malware', [$this, 'bulk_clean_malware']);
    ...
    /**
     * Safely parses JSON or form POST payloads across all web server environments.
     */
    private function get_json_input() {
        ...
        if (!empty($_POST)) {
            ...
            if (is_string($first_key) && str_starts_with(trim($first_key), '{')) {
                $json_parsed = json_decode($first_key, true);
    ...
    public function get_settings() {
        ...
        wp_send_json_success([
            ...
            'main_waf_api_url' => get_option('waf_fw_main_waf_api_url', 'http://217.15.170.82/api'),
            'main_waf_admin_url' => get_option('waf_fw_main_waf_admin_url', 'http://217.15.170.82'),
    ...
    public function save_settings() {
        ...
        if (empty($ml_url) || strpos($ml_url, 'onrender.com') !== false || strpos($ml_url, 'mdefender-pro.io') !== false) {
            update_option('waf_fw_ml_api_url', 'http://217.15.170.82');
        }
        ...
        if ($connection !== null) {
            ...
            $response['connected'] = get_option('waf_fw_connected', 'no') === 'yes';
            $response['connection_message'] = $response['connected'] ? 'Cloud service connected.' : 'Not connected.';
        }
    ...
    public function diff_core_file() {
        $this->check_access();
        ...
        if (!file_exists($path)) {
            wp_send_json_error(['message' => 'Local file does not exist on disk']);
        }

        $local_content = @file_get_contents($path);
        if ($local_content === false) {
            wp_send_json_error(['message' => 'Unable to read local file contents']);
        }
        ...
        if (empty($original_content)) {
            wp_send_json_error(['message' => 'Could not fetch original WordPress core file from WordPress.org repository.']);
        }
        ...
        wp_send_json_success([
            ...
            'local_hash' => md5($local_content),
        ]);
    }

    public function bulk_restore_core_files() {
        $this->check_access();
        $this->verify_nonce();
        ...
        if (empty($files) || !is_array($files)) {
            global $wpdb;
            $table_queue = $wpdb->prefix . WAF_FW_TABLE_SCAN_QUEUE;
            $last_scan = $wpdb->get_row("SELECT results FROM $table_queue WHERE status IN ('completed', 'completed_with_issues') ORDER BY id DESC LIMIT 1");
            if ($last_scan && !empty($last_scan->results)) {
                $results = json_decode($last_scan->results, true);
                if (!empty($results['malware_scan']['wp_checksums']['modified_files'])) {
                    foreach ($results['malware_scan']['wp_checksums']['modified_files'] as $mf) {
                        $files[] = $mf['file'];
                    }
                }
            }
        ...
        if (empty($files)) {
            wp_send_json_error(['message' => 'No repairable core files found to restore.']);
        }
    ...
    public function bulk_clean_malware() {
        $this->check_access();
        $this->verify_nonce();
        $input = $this->get_json_input();
        $files = $input['files'] ?? [];
        ...
        if (empty($files) || !is_array($files)) {
            ...
            if ($last_scan && !empty($last_scan->results)) {
                ...
                if (!empty($results['ml_malware_scan']['malicious_files'])) {
                    foreach ($results['ml_malware_scan']['malicious_files'] as $mf) {
                        $files[] = $mf['file'];
                    }
                }
            }
        ...
        if (empty($files)) {
            wp_send_json_error(['message' => 'No deletable malware files found to clean.']);
        }
